...
> Shared runtime internals for `@nrbx/react`.

This package is **private**. It is an internal runtime dependency of the `@nrbx/react` package family and is not intended to be consumed directly by application code.

It contains the React symbol constants, element markers (`Event`, `Change`, `Tag`), invariant and error formatting helpers, shared React types, and the feature-flag and host-config shims used across the runtime.
