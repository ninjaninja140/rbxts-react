...
## @nrbx/react-reconciler

> The React 19 fiber reconciler for Roblox, ported to roblox-ts.

This package is **private**. It is an internal runtime dependency of `@nrbx/react` and `@nrbx/react-roblox` and is not intended to be consumed directly by application code.

It contains the fiber work loop, scheduler integration, hook dispatcher, commit phase, and the host-config boundary that `@nrbx/react-roblox` implements to talk to Roblox instances.
